...
from portfolio.components.cards import BentoGrid, ConsolePanel, SkillPill, link_button
...
def ProjectCard(page: ft.Page, project: dict[str, Any]) -> ft.Control:
    ...
    return ConsolePanel(
        ...
        title=f"{project.get('category', 'project').lower()} - {project.get('status', 'case study').lower()}",
        bgcolor="#111827",
    )
